...
创建的目录结构（遵循 [XDG Base Directory Specification](https://specifications.freedesktop.org/basedir-spec/latest/)）：
```
...
```bash
# 从 GitHub 升级到最新版本（替换 @vX.Y.Z 为目标版本）
pip install --upgrade git+https://github.com/lbwds/issue-tracker-cli.git@v2.2.0
```

数据目录和配置文件不受影响，升级后可直接使用。

### 1.3 卸载

卸载分两步：移除工具包本身，和删除数据目录。
...
# 如果曾自定义过 XDG_CONFIG_HOME / XDG_DATA_HOME，使用对应路径
rm -rf "$XDG_CONFIG_HOME/issue-tracker"
rm -rf "$XDG_DATA_HOME/issue-tracker"
```

> **注意**：如果希望保留数据，在删除前自行备份 `~/.local/share/issue-tracker/` 目录。

---

### 1.4 推荐方式：项目目录配置

在项目根目录使用 `iss-project` 引导创建配置文件 `issue-tracker.yaml`：
...
也可使用 `iss-ui` 管理全局配置和项目：

```bash
iss-ui   # 打开菜单，可查看路径、编辑全局配置、备份/恢复项目等
```
...
**默认输出**: `$XDG_DATA_HOME/issue-tracker/exports/xxx.md`（配置文件中指定）
...
如果希望修改配置或数据的存储位置，在 `~/.bashrc` 或 `~/.zshrc` 中设置标准 XDG 变量：

```bash
export XDG_CONFIG_HOME=$HOME/.config       # 默认值，通常无需修改
export XDG_DATA_HOME=$HOME/.local/share    # 默认值，通常无需修改
```
...
在项目目录中运行 `iss-project`，按引导式流程填写信息：
...
#   项目 ID (纯数字, 如 002):    002
#   项目名称:                    NewProject
#   优先级列表 [P0,P1,P2,P3]:   (回车接受默认)
#   状态列表 [...]:              (回车接受默认)
#   启用 GitHub 同步? [y/N]:     y
#   ...
#   确认写入? [y/N]:             y
#   ✓ 已创建: /path/to/new-project/issue-tracker.yaml
```

生成的 `issue-tracker.yaml` 自动包含所有配置，也可随时用 `iss-project` 再次编辑。
...
| 文件/目录 | 路径 | 说明 |
|-----------|------|------|
| 项目配置 | `<项目目录>/issue-tracker.yaml` | 项目本地配置（由 `iss-project` 创建） |
| 全局配置 | `$XDG_CONFIG_HOME/issue-tracker/globals.yaml` | 全局默认值（由 `iss-ui` 管理） |
| 配置目录 | `$XDG_CONFIG_HOME/issue-tracker/` | 项目配置文件存储（默认 `~/.config/issue-tracker/`） |
| 数据库 | `$XDG_DATA_HOME/issue-tracker/` | 本地数据库存储（默认 `~/.local/share/issue-tracker/`） |
| 导出文件 | `$XDG_DATA_HOME/issue-tracker/exports/` | Markdown 导出文件 |
| 备份 | `$XDG_DATA_HOME/issue-tracker/backups/` | 项目备份存储（tar.gz，由 `iss-ui` 管理） |
